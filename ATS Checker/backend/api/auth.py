...
def _verify_token(token: str) -> dict:
    ...
    if alg == 'HS256':
        from backend.core.config import JWT_SECRET, SUPABASE_JWT_SECRET
        if JWT_SECRET:
            try:
                return jwt.decode(
                    token,
        ...
        if not SUPABASE_JWT_SECRET:
            raise jwt.InvalidTokenError(
                'HS256 token received but neither JWT_SECRET nor SUPABASE_JWT_SECRET is configured'
            )
        return jwt.decode(
            token,
            SUPABASE_JWT_SECRET,
...
def get_current_user(
    ...
    if not SUPABASE_URL and not SUPABASE_JWT_SECRET and not JWT_SECRET:
        logger.error('Neither SUPABASE_URL, SUPABASE_JWT_SECRET nor JWT_SECRET configured — cannot verify tokens')
        raise HTTPException(
    ...
    if not user_id:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail='Token missing subject or user ID claim',
        )
    return str(user_id)
